...
def plot_world_map(lons, lats, data, metadata, plotpath):
    ...
    plttitle = 'JEDI FV3 %s of variable %s on surface level %d by %s' % (metadata['datatype'],metadata['var'],metadata['level'],os.environ['LOGNAME'])
    if metadata['datatype'] == 'inc':
        vmax = vmin * -1.0
        cmap = 'bwr'
    for t in range(0,6):
...
def read_var(datapath, geopath, varname, level):
...
def gen_figure(inpath, geopath, outpath, datatype, varname, level):
    # read the files to get the 2D array to plot
    data, lons, lats = read_var(inpath, geopath, varname, level)
    plotpath = outpath+'/%s_%s_%d.png' % (datatype, varname, level)
...
if __name__ == "__main__":
    ...
    ap.add_argument('-t', '--type', help="inc, bkg, or anl", default="inc")
    ap.add_argument('-v', '--variable', help="variable name to plot", required=True)
    ap.add_argument('-l', '--level', help="level to plot", required=True)
    ap.add_argument('-g', '--geoin', help="path to prefix of input files with geolat/geolon", required=True)
    MyArgs = ap.parse_args()
    gen_figure(MyArgs.input, MyArgs.geoin, MyArgs.output, MyArgs.type, MyArgs.variable, int(MyArgs.level))
